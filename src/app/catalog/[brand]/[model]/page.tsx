...
export default function ProductPage() {
  ...
  if (!brand || !model) return <div className="py-20 text-center"><h1 className="text-xl font-bold">Not found</h1><Link href="/catalog" className="mt-3 inline-block text-gold text-sm">Catalog</Link></div>;
      ...
      <div className="border-b border-border/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <nav className="text-xs text-text-dim">
            <Link href="/catalog" className="hover:text-gold transition-colors">Catalog</Link><span className="mx-2 text-border">/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
          <div className="space-y-4 lg:sticky lg:top-24 lg:self-start">
            ...
            <div className="aspect-[4/3] glass-card rounded-xl relative overflow-hidden p-6 lg:p-8">
              ...
              <div className="absolute top-4 left-4 text-[10px] uppercase tracking-[0.2em] text-gold/60 font-semibold">Preview</div>
            </div>
            ...
            <div className="flex gap-3">
              <div className="glass-card rounded-lg px-4 py-3 flex items-center gap-3 flex-1">
                ...
                <div className="min-w-0">
                  <div className="text-[10px] uppercase tracking-wider text-text-faint">Mat</div>
              ...
              <div className="glass-card rounded-lg px-4 py-3 flex items-center gap-3 flex-1">
                ...
                <div className="min-w-0">
                  <div className="text-[10px] uppercase tracking-wider text-text-faint">Edge</div>
          ...
          <div>
            ...
            <p className="text-text-dim text-sm mt-1">{model.bodyType} · EVA floor mats</p>
            <div className="mt-4 flex items-baseline gap-2">
              <span className="text-gold text-3xl font-bold">{formatPrice(unitPrice)}</span>
            </div>
            <div className="mt-10 space-y-8">
              <div>
                <h3 className="section-label text-[10px] mb-3">1 — Year</h3>
              ...
              <div>
                <h3 className="section-label text-[10px] mb-3">2 — Set</h3>
              ...
              <div>
                <h3 className="section-label text-[10px] mb-3">3 — Mat color: <span className="normal-case text-text">{color.name}</span></h3>
                <div className="flex gap-3">
              ...
              <div>
                <h3 className="section-label text-[10px] mb-3">4 — Edge: <span className="normal-case text-text">{edge.name}</span></h3>
                <div className="flex gap-3">
              ...
              {bdg && (
                <div>
                  <h3 className="section-label text-[10px] mb-3">5 — Badge</h3>
                  <label className="flex items-center gap-4 cursor-pointer glass-card glow-hover rounded-xl p-4">
                    ...
                    <div><span className="text-text text-sm font-medium">{brand.name} badge</span><p className="text-text-dim text-xs mt-0.5">Metal brand logo</p></div>
                  </label>
                </div>
              )}
              <button onClick={add} className={`w-full py-4 rounded-xl text-sm font-semibold tracking-wider uppercase transition-all duration-300 ${added ? "bg-success text-bg" : "bg-gradient-to-r from-gold to-gold-light text-bg shadow-[0_4px_24px_rgba(212,165,74,0.25)] hover:shadow-[0_6px_32px_rgba(212,165,74,0.4)]"}`}>
                {added ? "✓ Added!" : `Add to cart — ${formatPrice(unitPrice)}`}
              </button>
            </div>
          </div>
        </div>
