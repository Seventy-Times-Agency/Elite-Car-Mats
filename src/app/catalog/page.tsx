            ...
            >
              {brand.logo ? (
                <div className="w-16 h-12 mx-auto relative">
                  <Image src={brand.logo} alt={brand.name} fill className="object-contain opacity-50 group-hover:opacity-100 transition-opacity duration-300" sizes="64px" />
                </div>
              ) : (
                <div className="w-16 h-12 mx-auto flex items-center justify-center text-light-text group-hover:text-gold text-xl font-bold transition-colors">{brand.name.charAt(0)}</div>
              )}
