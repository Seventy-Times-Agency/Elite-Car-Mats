  ...
  return (
    <div className="py-16 lg:py-24 min-h-screen">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12 pb-12 border-b border-dark-border">
          <div className="text-4xl font-bold text-text-primary">5.0</div>
          <div className="flex justify-center mt-1"><Stars rating={5} /></div>
          <p className="text-text-muted text-xs mt-2">{mockReviews.length} отзывов</p>
        </div>
        <div className="divide-y divide-dark-border">
          {mockReviews.map((r) => (
            <div key={r.id} className="py-8">
              <div className="flex items-center justify-between mb-3">
                <div><div className="text-text-primary text-sm font-medium">{r.customerName}</div><div className="text-text-muted text-xs">{r.carModel}</div></div>
                <span className="text-text-muted text-xs">{new Date(r.createdAt).toLocaleDateString("ru-RU")}</span>
              </div>
